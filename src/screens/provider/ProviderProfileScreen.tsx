...
export const ProviderProfileScreen: React.FC = () => {
  ...
  const menuItems: MenuItem[] = useMemo(
    () => [
      { id: '1', label: t('profile.favourites'), icon: 'heart-outline', onPress: () => navigation.navigate('Favourites') },
      { id: '2', label: t('providerTime.title'), icon: 'time-outline', onPress: () => navigation.navigate('ProviderTime') },
      { id: '3', label: t('providerWallet.title'), icon: 'wallet-outline', onPress: () => navigation.navigate('ProviderWallet') },
      { id: '4', label: t('profile.notifications'), icon: 'notifications-outline', onPress: () => navigation.navigate('Notifications') },
      { id: '5', label: t('profile.security'), icon: 'shield-checkmark-outline' },
      { id: '6', label: t('profile.language'), icon: 'language-outline', onPress: () => navigation.navigate('LanguageSettings') },
      { id: '7', label: t('profile.helpCenter'), icon: 'help-circle-outline', onPress: () => navigation.navigate('HelpCenter') },
      { id: '8', label: t('inviteFriends.menuLabel'), icon: 'people-outline' },
      { id: '9', label: t('profile.signOut'), icon: 'log-out-outline', onPress: handleSignOut, danger: true },
    ],
    [navigation, t],
  );
...
const styles = StyleSheet.create({
  ...
  plusBadge: { position: 'absolute', bottom: 0, end: 0, width: 28, height: 28, borderRadius: 14, backgroundColor: Colors.primary, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: '#FFFFFF' },
  ...
  email: { fontSize: 14, fontFamily: FontFamily.outfit.regular, color: '#9AA0AE', marginBottom: 28 },
